...
def process_npy():
    for tag in ['MR', 'CT']:
        ...
        for path in tqdm(glob.glob(os.path.join(base_dir, tag, f'imagesTr', '*.nii.gz'))):
            ...
            print(label_id,np.unique(label_arr))
            ...
            print(img_id,max(np.unique(image_arr)))
            ...
            print(img_id,max(np.unique(image_arr)))
            if "srr" in img_id:
                image_arr = (image_arr + np.ones(image_arr.shape))*1000 
                print(img_id,max(np.unique(image_arr)))
            upper_bound_intensity_level = np.percentile(image_arr, 98)
            image_arr = np.clip(image_arr,a_min=0, a_max=upper_bound_intensity_level)
            ...
            image_arr = zoom(image_arr, [144/dn, 144/hn, 144/wn], order=3)
            label_arr = zoom(label_arr, [144/dn, 144/hn, 144/wn], order=0)

            label_arr = np.round(label_arr).astype(np.int8)
            ...
            # np.save(
            #     os.path.join(config.save_dir, 'niigz', f'{img_id}.nii.gz'),
            #     image_arr
            # )
            # np.save(
            #     os.path.join(config.save_dir, 'niigz', f'{label_id}.nii.gz'),
            #     label_arr
            # )
...
def process_split_fully(train_val_ratio=0.8):
    ...
    for path in tqdm(glob.glob(os.path.join(config.save_dir, 'npy', '*_image.npy'))):
        ...
        if 'mr' in img_id and "ct2mr" not in img_id:
