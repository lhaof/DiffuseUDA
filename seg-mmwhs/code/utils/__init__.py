...
def read_data(data_id, task, normalize=False):
    ...
    # 修改文件路径
    im_path = os.path.join(config.save_dir, 'processed', f'{data_id}_image.nii.gz')
...
def test_all_case_dsc(task, net, ids_list, num_classes, patch_size, stride_xy, stride_z, test_save_path=None):
    for data_id in tqdm(ids_list):
