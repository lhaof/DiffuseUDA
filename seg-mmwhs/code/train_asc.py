...
# from deformable_aug import aug_mask_and_img
...
def FDA_source_to_target(src_img, trg_img, L=None):
    ...
    if L is None:
        ...
        L = max(L, 0.1)
        uncertainty_factor = random.uniform(0.5, 1.5)
...
# def process_batch(volume_batch, label_batch):
#     batch_size = volume_batch.size(0)
    
#     processed_volumes = []
#     processed_labels = []

#     for i in range(batch_size):
#         volume_sample = volume_batch[i]  # 取出单个样本，形状为 (channels, depth, height, width)
#         label_sample = label_batch[i][0]  # 取出单个标签，形状为 (depth, height, width)

#         # 假设我们只处理第一个通道，如果有多个通道
#         volume_sample = volume_sample[0]  # 现在形状为 (depth, height, width)
#         # 调用 aug_mask_and_img 函数
#         processed_mask, processed_image = aug_mask_and_img(label_sample, volume_sample)

#         # 将处理后的 mask 和 image 加入列表
#         processed_volumes.append(torch.tensor(processed_image).cuda())
#         processed_labels.append(torch.tensor(processed_mask).cuda())

#     # 将列表转换为张量
#     processed_volumes = torch.stack(processed_volumes).cuda().unsqueeze(1).float()
#     processed_labels = torch.stack(processed_labels).cuda().unsqueeze(1)
...
#     save_path = 'runs/' + 'asc_diffuse_x4_128' + '/'
...
#     valid_loader_seg = DataLoader(valid_seg, batch_size=int(args.batch_size/2), shuffle=True, num_workers=4, pin_memory=True) 
...
#             volume_batch, label_batch_s_ori = torch.cat((sampled_batch_s['image'],sampled_batch_t['image'])), sampled_batch_s['mask']
#             volume_batch, label_batch_s = volume_batch, label_batch_s_ori
#             volume_batch = volume_batch.cuda()
#             volume_batch_t = volume_batch[batch_size_half:]
#             volume_batch_s = volume_batch[:batch_size_half]

#             # # 处理 volume_batch_s 和 label_batch_s
#             # volume_batch_s, label_batch_s = process_batch(volume_batch_s, label_batch_s_ori)

#             # # 处理 volume_batch_t 和 label_batch_s 的后半部分
#             # volume_batch_t, processed_labels_t = process_batch(volume_batch_t, label_batch_s_ori)
